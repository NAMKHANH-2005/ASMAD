...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public ArrayList<Item> getProductsForMonth(long userId, String yearMonth) {
        ArrayList<Item> items = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        String query = "SELECT * FROM " + TABLE_PRODUCT + " WHERE " + TABLE_PRODUCT_COLUMN_USER_ID + " = ? AND "
                + "strftime('%Y-%m', " + TABLE_PRODUCT_COLUMN_CREATED_AT + ") = ?";
        Cursor cursor = db.rawQuery(query, new String[]{String.valueOf(userId), yearMonth});

        if (cursor != null && cursor.moveToFirst()) {
            do {
                Item item = new Item(
                        cursor.getInt(cursor.getColumnIndexOrThrow(TABLE_PRODUCT_COLUMN_ID)),
                        cursor.getString(cursor.getColumnIndexOrThrow(TABLE_PRODUCT_COLUMN_NAME)),
                        cursor.getInt(cursor.getColumnIndexOrThrow(TABLE_PRODUCT_COLUMN_QUANTITY)),
                        cursor.getDouble(cursor.getColumnIndexOrThrow(TABLE_PRODUCT_COLUMN_PRICE)),
                        cursor.getString(cursor.getColumnIndexOrThrow(TABLE_PRODUCT_COLUMN_CREATED_AT))
                );
                items.add(item);
            } while (cursor.moveToNext());
        }
